...
export function generateCV(data: FormRequest, outputPath: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    doc
      .fontSize(10)
      .fillColor(TEXT_COLOR)
      .text(`${data["personal_info.email"]} | ${data["personal_info.phone"]} | ${data["personal_info.linkedin"]}`, {
        align: 'center'
      })
      .moveDown(2);
    ...
    doc
      ...
      .text(data["professional_summary.summary"])
      .moveDown(1);
    ...
    doc
      .fontSize(TEXT_FONT_SIZE)
      .fillColor(PRIMARY_COLOR)
      .text(`${data["work_experience.job_title"]} - ${data["work_experience.company"]}`, { continued: false });
    doc
      .fillColor(TEXT_COLOR)
      .text(`${data["work_experience.period"]}`)
      .moveDown(0.5);

    doc
      .fontSize(TEXT_FONT_SIZE)
      .text(data["work_experience.achievements"])
      .moveDown(1);
...
export function generateCoverLetter(data: FormRequest, outputPath: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    doc
      .text("Hiring Manager")
      .text(`${data["letter_customization.target_company"]}`)
      .moveDown(2);

    // Cuerpo
    doc.text(`Dear Hiring Team,`).moveDown(1);

    doc.fontSize(TEXT_FONT_SIZE).text(
    ...
    doc.text(`I would welcome the opportunity to discuss how my skills align with your needs. Thank you for your consideration.`).moveDown(2);
